...
std::vector<std::vector<Edge>> bfs_tree(const VRP &vrp, std::vector<std::vector<Edge>> graph, int k)
{
    // for(auto it: vrp.node){
    //   std::cout<<it.x<<" "<<it.y<<std::endl;
    // }
    int n = graph.size();
    std::vector<bool> visited(n, false);
    std::vector<std::vector<Edge>> spanningTreeEdges(n,std::vector<Edge>());  // Edges of the spanning tree
    queue<node_t> q;
    node_t start = 1;
    ...
    while (!q.empty()) {
        int node = q.front();
        q.pop();
        priority_queue<std::pair<weight_t,node_t>> maxh;
        for(Edge neighbor: graph[node]){
          if(!visited[neighbor.to]){
            maxh.push({neighbor.length,neighbor.to});
            if(maxh.size()>k){
              maxh.pop();
            }
          }
        ...
        while(!maxh.empty()){
          std::pair<weight_t,node_t> p = maxh.top(); maxh.pop();
          spanningTreeEdges[node].push_back(Edge(p.second,p.first));
          spanningTreeEdges[p.second].push_back(Edge(node,p.first));
...
std::vector<std::vector<Edge>> make_local_mst(const VRP &vrp, std::vector<std::vector<Edge>> graph)
{
  ...
  for (int i = 0; i < grid.size(); ++i) {
        ...
        int edgesAdded = 0;
        for (int j = 1; j < n; ++j) {
            double dist = euclideanDistance(grid[i][0], grid[i][j]);
            pq.push({dist, {0, j}});
        }
        ...
        while (!pq.empty() && edgesAdded < n - 1) {
            ...
            if (!visited[to]) {
                visited[to] = true;
                edgesAdded++;
                int globalFrom = coord[{i,from}];
                int globalTo = coord[{i,to}];
                localMSTs[globalFrom].push_back(Edge(globalTo, weight));
                localMSTs[globalTo].push_back(Edge(globalFrom, weight));
                for (int k = 0; k < n; ++k) {
                    if (!visited[k]) {
                        double dist = euclideanDistance(grid[i][to], grid[i][k]);
                        pq.push({dist, {to, k}});
                    }
                }
            }
    ...
    for (int row = 0; row < gridSize; ++row) {
        for (int col = 0; col < gridSize; ++col) {
            int cellIndex = row * gridSize + col;
            const auto& pointsInCell = grid[cellIndex];
            if (pointsInCell.empty()) continue;

            std::vector<int> neighbors;
            if (col < gridSize - 1) neighbors.push_back(row * gridSize + (col + 1));
            if (row < gridSize - 1) neighbors.push_back((row + 1) * gridSize + col);

            for (int neighborIndex : neighbors) {
                const auto& neighborPoints = grid[neighborIndex];
                if (neighborPoints.empty()) continue;

                double minDist = 1e9;
                std::pair<int, int> minEdge = {-1, -1};

                for (size_t i = 0; i < pointsInCell.size(); ++i) {
                    const auto& p1 = pointsInCell[i];
                    for (size_t j = 0; j < neighborPoints.size(); ++j) {
                        const auto& p2 = neighborPoints[j];
                        double dist = euclideanDistance(p1, p2);
                        if (dist < minDist) {
                            minDist = dist;
                            minEdge = {coord[{cellIndex, i}], coord[{neighborIndex, j}]};
                        }
                    }
                }

                if (minEdge.first != -1) {
                    localMSTs[minEdge.first].push_back(Edge(minEdge.second, minDist));
                    localMSTs[minEdge.second].push_back(Edge(minEdge.first, minDist));
                }
            }
        }
    }
...
int main(int argc, char *argv[]) {
  ...
  // int k=10;
  // auto mstG = bfs_tree(vrp, cG, k);
  auto mstG = make_local_mst(vrp, cG);
